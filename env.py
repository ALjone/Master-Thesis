...
class BlackBox(gym.Env):
    def __init__(self, resolution = 11, domain = [2, 12, 2, 12], u_v_range = 4, num_init_points = 2, T = 60):
        ...
        #Get bin size
        x_bin_size = x_bin_size/2#(self.x_max-self.x_min)/(self.resolution*2)
        y_bin_size = y_bin_size/2#(self.y_max-self.y_min)/(self.resolution*2)
        print(x_bin_size)
        ...
        print("x bins right:", self.x_bins_right)
        print("x bins left:", self.x_bins_left)
        print("y bins right:", self.y_bins_right)
        print("y bins left:", self.y_bins_left)
    ...
    def reset(self) -> np.ndarray:
        ...
        #self.x_sign = np.random.choice([-1, 1])
        #self.y_sign = np.random.choice([-1, 1])
    ...
    def _find_indices(self, x, y):
        indices = []
        for x_ind in [np.digitize(x, x_bin) for x_bin in (self.x_bins_left, self.x_bins_right)]:
            for y_ind in [np.digitize(y, y_bin) for y_bin in (self.y_bins_left, self.y_bins_right)]:
