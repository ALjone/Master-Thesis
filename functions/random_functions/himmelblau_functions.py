...
class RandomHimmelblau:
    ...
    def _make_matrix_2d(self, size, params):
        ...
        if size == 1:
            himmelblau = torch.log2((X_rot**2 + Y_rot - params["a"])**2 + (X_rot + Y_rot**2 - params["b"])**2)
        else:
            himmelblau = torch.log2((X_rot**2 + Y_rot - params["a"][:size])**2 + (X_rot + Y_rot**2 - params["b"][:size])**2)
