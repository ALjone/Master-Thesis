...
class RandomBranin:
    def __init__(self, config) -> None:
        self.range_x1 = [-5, 10]  # range for x1 in the Branin function
        self.range_x2 = [0, 15]  # range for x2 in the Branin function
    ...
    def _make_matrix_2d(self, size, params):
        ...
        branin = self.a * (X2_rot - self.b * X1_rot**2 + self.c * X1_rot - self.r)**2 + self.s * (1 - self.t) * torch.cos(X1_rot) + self.s
        # Normalize the function
        branin = (1 / self.std) * (branin - self.mean)
        # Maximize the function
        matrix = (-branin).to(torch.float32)
