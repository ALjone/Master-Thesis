...
class RandomHimmelblau:
    ...
    def get_params(self, size):
        a = rand(self.a_vals[0], self.a_vals[1], size=(size+1))[:-1, None, None]
    ...
    def _make_matrix_2d(self, size, params):
        ...
        if size == 1:
            himmelblau = torch.log10((X_rot**2 + Y_rot - params["a"])**2 + (X_rot + Y_rot**2 - params["b"])**2)
        else:
            himmelblau = torch.log10((X_rot**2 + Y_rot - params["a"][:size])**2 + (X_rot + Y_rot**2 - params["b"][:size])**2)

        # Shift and scale Himmelblau function so that its minimum is 0 and maximum is 1
        himmelblau -= himmelblau.min()
        himmelblau /= himmelblau.max()

        # Invert the function, so we are now maximizing it instead of minimizing it
        matrix = (1 - himmelblau).to(torch.float32)
