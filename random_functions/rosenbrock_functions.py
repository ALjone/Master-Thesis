...
class RandomRosenbrock:
    ...
    def _make_matrix_2d(self, size, params):
        ...
        if size == 1:
            matrix = torch.log10(1 + 1 / ((params["a"] - X_rot) ** 2 + params["b"] * (Y_rot - X_rot**2) ** 2)).to(torch.float32)   
        else:
            matrix = torch.log10(1 + 1 / ((params["a"][:size] - X_rot) ** 2 + params["b"][:size] * (Y_rot - X_rot**2) ** 2)).to(torch.float32)
