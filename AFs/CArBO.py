...
class CArBO:
    ...
    def __call__(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        std = observations[:, 1]
        time = observations[:, 2]
        remaning_time = 1 - (observations[:, 3, 0, 0] if self.dims == 2 else observations[:, 3, 0, 0, 0])
        for _ in range(self.dims):
                remaning_time = remaning_time.unsqueeze(1)
        with torch.no_grad():
        ...
        return (EI/(time**remaning_time)).to(torch.device("cpu"))
