...
from agents.pointwise import Agent as Agent
...
def get_config():
    ...
    print("Running with minibatch size:", config.minibatch_size)
...
if __name__ == "__main__":
    ...
    next_img_obs = env.reset()
    next_done = torch.zeros(config.batch_size).to(device)
    num_updates = config.total_timesteps // (config.batch_size*config.num_steps)
    pbar =  tqdm(total=config.total_timesteps, desc = "Steps done")
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, config.num_steps):
            ...
            next_img_obs, reward, next_done, info = env.step(action_before_tanh) #, False if isinstance(agent, tanh_Agent) else True)
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_img_obs).reshape(1, -1)
        ...
        b_inds = np.arange(b_values.shape[0])
        clipfracs = []
        agent.train()
        for epoch in range(config.update_epochs):
            
            for start in range(0, np.max(b_inds), config.minibatch_size):
                end = start + config.minibatch_size
                mb_inds = b_inds[start:end]

                _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_img_obs[mb_inds], b_actions_before_tanh.long()[mb_inds])
        ...
        for name, value in episodic_returns.items():
            returns += value
            writer.add_scalar(f"individual_performance/return_{name}", np.mean(value), global_step)
        for name, value in episodic_lengths.items():
            lenghts += value
            writer.add_scalar(f"individual_performance/length_{name}", np.mean(value), global_step)
        for name, value in episodic_peaks.items():
            peaks += value
            writer.add_scalar(f"individual_performance/peak_{name}", np.mean(value), global_step)
        ...
        pbar.update(config.batch_size*config.num_steps)
